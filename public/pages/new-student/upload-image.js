...
const targetFolder = `Student_pictures/${db_year}/${db_grade}`;
...
const name_en = document.getElementById('fill2');
...
imageFile.addEventListener("change", function () {
    ...
    formData.append("public_id", `${set_name}_pic_${combinedNumber}`);
...
imageFile2.addEventListener("change", function () {
    ...
    const set_name = `${name_en.value}_birth`
    ...
    formData.append("upload_preset", uploadPreset);
    formData.append("folder", targetFolder);
    formData.append("public_id", `${set_name}_iden__${combinedNumber}`);
    const xhr = new XMLHttpRequest();
    xhr.upload.addEventListener("progress", function (e) {
        if (e.lengthComputable) {
            const percentComplete = Math.round((e.loaded * 100) / e.total);
