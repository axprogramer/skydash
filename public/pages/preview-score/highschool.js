...
export function highschool(datas) {
        ...
        if (all_grade.includes(dbGrade)) {
            ...
            if (dbMonth == '2nd Semester') {
                if (primary_g_p4.includes(dbGrade)) {
                    ...
                    <th class="border-dark">កុំព្យូទ័រ</th>
                    <th class="border-dark">គេហវិទ្យា</th>
                    <th class="border-dark">សេដ្ឋកិច្ច</th>
                    <th class="border-dark">កីឡា</th>
                    <th class="border-dark">ពិន្ទុសរុប</th>
                    <th class="border-dark">មធ្យមភាគ</th>
                    <th class="border-dark">ចំណាត់ថ្នាក់</th>
                    ...
                    <td class="border-dark text-center" contenteditable onclick="selectText(this)" data-eco_m2semester="${N0}" id="${noSp}eco_m2semester">${eco_m2semester}</td>
                    ...
                    <td class="border-dark text-center" style="color:blue;">${session}</td>
                            ...
                            setTimeout(() => {
                                ...
                                document.getElementById(`${noSp}eco_m2semester`).addEventListener('input', () => {
                                    var d = document.getElementById(`${noSp}eco_m2semester`).innerHTML;
                                    firebase
                                        .database()
                                        .ref(`SalaMOM/classes/` + `${dbyear}/` + `${grade_en}/` + id)
                                        .update({
                            ...
                            table.addEventListener("keydown", (event) => {
                                if (event.key === "Enter") {
                                    event.preventDefault();
                                    const currentCell = event.target;
                                    const currentIndex = parseInt(currentCell.dataset.eco_m2semester);
                                    const nextCell = document.querySelector(`[data-eco_m2semester="${currentIndex + 1}"]`);
                                    const focusedCell = document.activeElement;
                                    if (focusedCell && focusedCell.tagName === "TD") {
                                        const range = document.createRange();
                                        range.selectNodeContents(focusedCell);
